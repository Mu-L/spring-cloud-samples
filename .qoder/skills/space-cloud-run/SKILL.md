...
- 🔴如果未运行，则必须仔细在本地查找有没有安装
- 🔴必须首先查找的目录是`$HOME/ai-infra/nacos/standalone`，该目录没有找到则继续按如下方式查找
```bash
...
| AI 视觉理解 500            | 图片 URL 不可访问（百度图片会拒绝 Java UrlResource 请求），使用稳定可访问的 URL                                            |
