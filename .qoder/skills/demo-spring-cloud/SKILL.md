...
**步骤四：触发限流验证**（consumer_api 阈值 5 QPS，快速发送 10 次请求）
```bash
for i in $(seq 1 10); do
  echo -n "请求 $i: "
  curl -s 'http://localhost:8764/consumer-sample/hi?name=hongxi'
  echo ""
done
```
预期：前 5 次正常返回，后续返回 `{"code":444,"msg":"Sentinel gateway block"}`
