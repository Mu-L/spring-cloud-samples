...
description: >
  ...
  涵盖 17 个模块的完整演示流程。
tags: [spring-cloud, spring-cloud-alibaba, nacos, sentinel, seata, dubbo, grpc, rocketmq, stream, microservices, demo, spring-ai, rag, chatmemory, prompttemplate, vision, tool-calling, agent, trace]
---
...
> 使用 `demo-spring-cloud` skill 可执行深度验证脚本（verify-trace.sh / verify-nacos-config.sh / verify-sentinel-gateway.sh / verify-sentinel-app.sh / verify-stream.sh / verify-seata.sh），或查看各模块 README 了解详细用法
...
**执行一键验证脚本：**
```bash
bash .qoder/skills/demo-spring-cloud/verify-sentinel-app.sh
```
脚本会自动完成：检查前置服务 → 基线验证 → 清理旧规则 → 限流验证（资源名 `/hi`，QPS=1）→ 熔断降级验证（停止 provider，触发 Feign fallback + RestTemplate fallback）→ 恢复环境
...
> | 事务消息 | Consumer | StreamBridge → tx-topic → tx | 两阶段提交，本地事务成功后消息才被消费 |
