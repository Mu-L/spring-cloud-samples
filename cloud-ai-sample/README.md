...
**请求：**
```bash
curl "http://localhost:8888/ai/chat?message=你好，请介绍一下自己"
```
...
**请求：**
```bash
curl "http://localhost:8888/ai/chat/stream?message=讲一个简短的故事"
```
...
**请求：**
```bash
curl "http://localhost:8888/ai/extract?message=张三今年25岁，是一名软件工程师，邮箱是zhangsan@example.com"
```
...
**请求：**
```bash
curl -X POST "http://localhost:8888/ai/advanced/system-message?message=如何设计一个高并发的秒杀系统？"
```

**响应：**
```json
{
  "message": "如何设计一个高并发的秒杀系统？",
  "response": "作为资深Java架构师，我来为你详细分析..."
}
```
...
**请求：**
```bash
curl -X POST "http://localhost:8888/ai/advanced/few-shot?message=创建一个Map，key为用户名，value为年龄"
```

**响应：**
```json
{
  "message": "创建一个Map，key为用户名，value为年龄",
  "response": "Map<String, Integer> userAges = new HashMap<>();"
}
```
...
**请求：**
```bash
curl -X POST "http://localhost:8888/ai/advanced/conversation?message=你还记得我刚才说了什么吗？"
```
...
**请求：**
```bash
curl -X POST "http://localhost:8888/ai/advanced/creative?message=写一首关于春天的短诗"
```
...
**请求：**
```bash
curl -X POST -F "file=@/path/to/image.jpg" "http://localhost:8888/ai/vision/analyze-upload"
```
...
curl "http://localhost:8888/ai/tool/time?message=距离2027年春节还有多少天？"
```
...
# AI 自动调用 TimeTools
curl "http://localhost:8888/ai/tool/ask?message=现在几点了？"

# AI 自动调用 SearchTools
curl "http://localhost:8888/ai/tool/ask?message=什么是Spring AI？"
```
...
**请求：**
```bash
curl "http://localhost:8888/ai/agent/chat?message=北京今天的天气怎么样？适合出门吗？"
```
...
**请求：**
```bash
curl "http://localhost:8888/ai/agent/complex-task?message=我想去杭州旅游，帮我查一下杭州的天气，以及介绍一下杭州"
```
...
│   ├── service/
│   │   ├── ToolCallingService.java            # Tool Calling 服务
│   │   ├── VisionService.java                # 多模态图像处理服务
│   │   └── ProjectDemoService.java           # 项目演示 Agent 服务
│   ├── tool/
...
│   └── vo/
│       ├── AiResponse.java                   # AI 通用响应（message + response）
│       ├── AgentResult.java                  # Agent 响应（message + response + type）
│       ├── VisionResult.java                 # 视觉分析结果（imageUrl + response）
│       └── ImageComparisonResult.java        # 图片对比结果（imageUrls + response）
└── src/main/resources/
