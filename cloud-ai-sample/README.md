...
**请求：**
```bash
curl "http://localhost:8888/ai/extract?text=张三今年25岁，是一名软件工程师，邮箱是zhangsan@example.com"
```
...
**请求：**
```bash
curl -X POST "http://localhost:8888/ai/advanced/conversation?currentMessage=你还记得我刚才说了什么吗？"
```
...
**请求：**
```bash
curl "http://localhost:8888/ai/tool/weather?question=北京今天的天气怎么样？"
```
...
**请求：**
```bash
curl "http://localhost:8888/ai/tool/time?question=现在几点了？"
curl "http://localhost:8888/ai/tool/time?question=距离2027年春节还有多少天？"
```
...
curl "http://localhost:8888/ai/tool/ask?question=帮我查一下上海的天气"

# AI 自动调用 TimeTools
curl "http://localhost:8888/ai/tool/ask?question=现在几点了？"

# AI 自动调用 SearchTools
curl "http://localhost:8888/ai/tool/ask?question=什么是Spring AI？"
```
...
**请求：**
```bash
curl "http://localhost:8888/ai/agent/chat?question=北京今天的天气怎么样？适合出门吗？"
```
...
**请求：**
```bash
curl "http://localhost:8888/ai/agent/complex-task?task=我想去杭州旅游，帮我查一下杭州的天气，以及介绍一下杭州"
```
